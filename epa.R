...
all_seasons <- all_seasons[ which(all_seasons$Season == 2009),]
...
# drive t score flag (if zero, net with t+1 scoring result)
all_seasons$drive_t_score_flag <- ifelse(all_seasons$drive_points == 0, 0, 1)



# calculating drive_points_net
all_seasons$drive_points_net <- ifelse(all_seasons$drive_t_score_flag == 1, 
                                       all_seasons$drive_points, all_seasons$drive_points_t_plus_1*(-1))
